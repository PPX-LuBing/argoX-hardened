...
set_internal_ports() {
  VL_PORT=$PORT_START
  VM_PORT=$((PORT_START + 1))
  TR_PORT=$((PORT_START + 2))
  SH_PORT=$((PORT_START + 3))
  XRAY_FALLBACK_PORT=$((PORT_START + 4))
  XH_PORT=$((PORT_START + 5))
}
...
input_port_start() {
  ...
  while true; do
    ...
    if is_interactive; then
      if [ "$L" = 'C' ]; then
        reading "\n (7/9) 请输入内部协议起始端口（需要连续6个端口，默认 ${PORT_START_DEFAULT}）: " PORT_START
      else
        reading "\n (7/9) Please enter internal start port (6 consecutive ports required, default ${PORT_START_DEFAULT}): " PORT_START
      fi
    fi

    PORT_START=${PORT_START:-"$PORT_START_DEFAULT"}
    if ! [[ "$PORT_START" =~ ^[1-9][0-9]{1,4}$ && "$PORT_START" -ge 1000 && "$PORT_START" -le 65530 ]]; then
      [ "$L" = 'C' ] && warning "\n 端口范围必须为 1000-65530 \n" || warning "\n Port must be in range 1000-65530 \n"
      unset PORT_START
      continue
    fi
    ...
    local IN_USED=()
    local SS_SNAPSHOT
    SS_SNAPSHOT=$(ss -nltup)
    for p in "$VL_PORT" "$VM_PORT" "$TR_PORT" "$SH_PORT" "$XRAY_FALLBACK_PORT" "$XH_PORT"; do
      grep -q ":$p" <<< "$SS_SNAPSHOT" && IN_USED+=("$p")
    done

    if [ "${#IN_USED[@]}" -gt 0 ]; then
      [ "$L" = 'C' ] && warning "\n 内部端口被占用: ${IN_USED[*]} \n" || warning "\n Internal ports in use: ${IN_USED[*]} \n"
      unset PORT_START
      continue
    fi

    if [ "$REALITY_PORT" -ge "$VL_PORT" ] && [ "$REALITY_PORT" -le "$XH_PORT" ]; then
      [ "$L" = 'C' ] && warning "\n Reality 端口与内部端口段冲突，请重新选择 \n" || warning "\n Reality port conflicts with internal port range, please choose another start port \n"
      unset PORT_START
      continue
    fi
...
download_github_release_asset() {
  ...
  local ASSET_NAME=$2
  local OUTPUT_PATH=$3
  local RELEASE_JSON DOWNLOAD_URL DIGEST EXPECTED_SUM

  RELEASE_JSON=$(wget -qO- "https://api.github.com/repos/${REPO}/releases/latest") || return 1
  DOWNLOAD_URL=$(jq -r --arg ASSET "$ASSET_NAME" '.assets[] | select(.name==$ASSET) | .browser_download_url' <<< "$RELEASE_JSON" | head -n1)
  DIGEST=$(jq -r --arg ASSET "$ASSET_NAME" '.assets[] | select(.name==$ASSET) | .digest // empty' <<< "$RELEASE_JSON" | head -n1)
  [ -n "$DOWNLOAD_URL" ] || return 1

  wget -qO "$OUTPUT_PATH" "$DOWNLOAD_URL" || return 1

  if [[ "$DIGEST" =~ ^sha256:([0-9a-fA-F]{64})$ ]]; then
    EXPECTED_SUM="${BASH_REMATCH[1]}"
    echo "${EXPECTED_SUM}  ${OUTPUT_PATH}" | sha256sum -c - >/dev/null 2>&1 || return 1
  fi
}
...
check_install() {
  ...
  # 下载所需文件
  [[ ${STATUS[0]} = "$(text 26)" ]] && [ ! -s $WORK_DIR/cloudflared ] && { download_github_release_asset "cloudflare/cloudflared" "cloudflared-linux-$ARGO_ARCH" "$TEMP_DIR/cloudflared" && chmod +x $TEMP_DIR/cloudflared >/dev/null 2>&1; }&
  [[ ${STATUS[1]} = "$(text 26)" ]] && [ ! -s $WORK_DIR/xray ] && { download_github_release_asset "XTLS/Xray-core" "Xray-linux-$XRAY_ARCH.zip" "$TEMP_DIR/Xray.zip" && unzip -qo $TEMP_DIR/Xray.zip xray *.dat -d $TEMP_DIR >/dev/null 2>&1; }&
}
...
node_name_prefix() {
  ...
  printf '%s' "${EMOJI}${EMOJI:+ }"
}

build_node_name_default() {
  local FALLBACK_MODE=${1:-with_emoji}
  local PREFIX=''
  [ "$FALLBACK_MODE" != 'plain' ] && PREFIX=$(node_name_prefix)

  if [ -x "$(type -p hostname)" ]; then
    printf '%s%s' "$PREFIX" "$(hostname)"
  elif [ -s /etc/hostname ]; then
    printf '%s%s' "$PREFIX" "$(cat /etc/hostname)"
  else
    [ "$FALLBACK_MODE" = 'plain' ] && printf 'ArgoX' || printf '%sArgoX' "$PREFIX"
  fi
}
...
argo_variable() {
  ...
  if [ ! -d $WORK_DIR ]; then
    is_interactive && [ -z "$SERVER_IP" ] && reading "\n $(text 59) " SERVER_IP
  ...
  is_interactive && [ -z "$ARGO_DOMAIN" ] && reading "\n $(text 10) " ARGO_DOMAIN
  ARGO_DOMAIN=$(sed 's/[ ]*//g; s/:[ ]*//' <<< "$ARGO_DOMAIN")

  # 输入 ARGO_AUTH
  if is_interactive && [[ -n "$ARGO_DOMAIN" && -z "$ARGO_AUTH" ]]; then
    local a=5
...
xray_variable() {
  ...
  is_interactive && [ -z "$WS_PATH" ] && reading "\n $(text 13) " WS_PATH
  local a=5
  ...
  if [ -z "$NODE_NAME" ]; then
    ...
    NODE_NAME_DEFAULT=$(build_node_name_default plain)
    is_interactive && reading "\n $(text 49) " NODE_NAME
    [ -z "$NODE_NAME" ] && NODE_NAME="$NODE_NAME_DEFAULT" || NODE_NAME="$(node_name_prefix)${NODE_NAME}"
  fi
}
...
fast_install_variables() {
  ...
  local SS_SNAPSHOT
  SS_SNAPSHOT=$(ss -nltup)
  while grep -q ":$REALITY_PORT" <<< "$SS_SNAPSHOT"; do
  ...
  local INTERNAL_PORT_USED_COUNT=0
  local SS_SNAPSHOT
  while true; do
    SS_SNAPSHOT=$(ss -nltup)
    if ! grep -Eq ":(${VL_PORT}|${VM_PORT}|${TR_PORT}|${SH_PORT}|${XRAY_FALLBACK_PORT}|${XH_PORT})\b" <<< "$SS_SNAPSHOT" && ! { [ "$REALITY_PORT" -ge "$VL_PORT" ] && [ "$REALITY_PORT" -le "$XH_PORT" ]; }; then
      break
    fi
    PORT_START=$((PORT_START + 10))
    [ "$PORT_START" -gt 65530 ] && PORT_START="$PORT_START_DEFAULT"
    set_internal_ports
  ...
  check_system_ip
  local NODE_NAME_DEFAULT
  NODE_NAME_DEFAULT=$(build_node_name_default)
  [ -z "$NODE_NAME" ] && NODE_NAME="$NODE_NAME_DEFAULT" || NODE_NAME="$(node_name_prefix)${NODE_NAME}"
}
...
check_dependencies() {
  ...
  # 所有系统都需要的基本依赖
  local DEPS_CHECK=("wget" "ss" "unzip" "bash" "jq" "qrencode")
  local DEPS_INSTALL=("wget" "iproute2" "unzip" "bash" "jq" "qrencode")
...
json_nginx() {
  ...
  if [ -s "$WORK_DIR/inbound.json" ]; then
    ...
    local DETECT_FALLBACK_PORT
    DETECT_FALLBACK_PORT=$(awk '
      /"dest"[[:space:]]*:/ {
        p=$0
        gsub(/[^0-9]/, "", p)
        if (p != "") last_dest=p
    ...
    ' <<< "$JSON")
    if [[ "$DETECT_FALLBACK_PORT" =~ ^[0-9]+$ && "$DETECT_FALLBACK_PORT" -ge 1004 ]]; then
      PORT_START=${PORT_START:-$((DETECT_FALLBACK_PORT - 4))}
    fi
  fi
...
http {
 ...
 #   ~*Chrome|Firefox|Mozilla  /;                # 添加更多的分流规则
  ...
  server {
    listen 127.0.0.1:${XRAY_FALLBACK_PORT} proxy_protocol; # xray fallbacks
...
export_list() {
  ...
  # # 生成 vmess 文件
  VMESS="{ \"v\": \"2\", \"ps\": \"${NODE_NAME}-Vm\", \"add\": \"${SERVER}\", \"port\": \"443\", \"id\": \"${UUID}\", \"aid\": \"0\", \"scy\": \"none\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"${ARGO_DOMAIN}\", \"path\": \"/${WS_PATH}-vm?ed=2560\", \"tls\": \"tls\", \"sni\": \"${ARGO_DOMAIN}\", \"alpn\": \"\" }"
...
vless://${UUID}@${SERVER}:443?encryption=none&security=tls&type=xhttp&mode=auto&path=/${WS_PATH}-xh&sni=${ARGO_DOMAIN}#${NODE_NAME// /%20}-Xh
...
vless://${UUID}@${SERVER}:443?encryption=none&security=tls&sni=${ARGO_DOMAIN}&type=xhttp&mode=auto&path=%2F${WS_PATH}-xh#${NODE_NAME// /%20}-Xh
...
ss://$(echo -n "${SS_METHOD}:${UUID}" | base64 -w0)@${SERVER}:443?plugin=v2ray-plugin;mode%3Dwebsocket;host%3D${ARGO_DOMAIN};path%3D/${WS_PATH}-sh;tls%3Dtrue;servername%3D${ARGO_DOMAIN};skip-cert-verify%3Dfalse;mux%3D0#${NODE_NAME// /%20}-Sh"
  ...
  echo $SING_BOX_JSON | sed "s#\"<OUTBOUND_REPLACE>\"#$OUTBOUND_REPLACE#; s#\"<NODE_REPLACE>\"#$NODE_REPLACE#g" | jq > $WORK_DIR/subscribe/sing-box
...
$(text 67):
$(qrencode -t UTF8 "https://${ARGO_DOMAIN}/${UUID}/auto")
EOF
...
$(hint "$(echo "{ \"outbounds\":[ ${OUTBOUND_REPLACE%,} ] }" | jq)
...
change_cdn() {
  ...
  if grep -q '.' <<< "$CDN_CHOOSE"; then
    ...
    local ESC_CDN_NOW ESC_CDN_NEW
    ESC_CDN_NOW=$(sed 's/[][\\.^$*+?|(){}\/&]/\\&/g' <<< "$CDN_NOW")
    ESC_CDN_NEW=$(sed 's/[\\/&]/\\&/g' <<< "$CDN_NEW")
    sed -i "s/${ESC_CDN_NOW}/${ESC_CDN_NEW}/g" "$WORK_DIR/inbound.json"
  fi
...
version() {
  ...
  local ONLINE=$(wget -qO- "${GH_PROXY}https://api.github.com/repos/XTLS/Xray-core/releases/latest" | grep "tag_name" | sed "s@.*\"v\(.*\)\",@\1@g")
  [ -z "$ONLINE" ] && error " $(text 74) "
  ...
  if [ "${UPDATE[0],,}" = 'y' ]; then
    download_github_release_asset "cloudflare/cloudflared" "cloudflared-linux-$ARGO_ARCH" "$TEMP_DIR/cloudflared"
  ...
  if [ "${UPDATE[1],,}" = 'y' ]; then
    download_github_release_asset "XTLS/Xray-core" "Xray-linux-$XRAY_ARCH.zip" "$TEMP_DIR/Xray-linux-$XRAY_ARCH.zip"
...
menu_setting() {
  ...
  if [[ "${STATUS[*]}" =~ $(text 27)|$(text 28) ]]; then
    if [ -s $WORK_DIR/cloudflared ]; then
      ...
      local ARGO_PID=$(awk '/\/etc\/argox\/cloudflared/{print $1; exit}' <<< "$PS_LIST")
    ...
    ACTION[7]() { run_external_tool_notice bbr; exit; }
...
check_system_info
check_dependencies
is_interactive && check_system_ip
check_install
menu_setting
